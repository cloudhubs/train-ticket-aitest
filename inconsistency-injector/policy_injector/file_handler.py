...
def find_all_config_files(root_dir):
    ...
    for service_dir in start_path.iterdir():
        if service_dir.is_dir() and service_dir.name.startswith("ts-"):
            config_path = service_dir / "src" / "main" / "resources" / "application.yml"
...
def restore_all_backups(root_dir):
    ...
    for service_dir in start_path.iterdir():
        if service_dir.is_dir() and service_dir.name.startswith("ts-"):
            backup_path = service_dir / "src" / "main" / "resources" / "application.yml.bak"
