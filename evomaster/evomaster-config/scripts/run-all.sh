...
# =============================================================================
# Run EvoMaster in black-box mode for all Train Ticket services
# =============================================================================
# Usage:
#   ./run-all.sh [role]
#
# Examples:
#   ./run-all.sh             # runs admin + user + none for all services
...
# Services to test: derived from available spec files
mapfile -t SERVICES < <(
    for f in "$SPEC_DIR"/*-openapi.json; do
        basename "$f" -openapi.json
    done | sort
)

# Roles to run — specific role via $1, otherwise all three
if [ -n "$1" ]; then
    ROLES=("$1")
else
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  EvoMaster — Train Ticket Experiment${NC}"
echo -e "${BLUE}============================================${NC}"
echo -e "${YELLOW}Start:${NC}            $EXPERIMENT_START"
echo -e "${YELLOW}EvoMaster:${NC}        ${EVOMASTER_VERSION}"
...
for ROLE in "${ROLES[@]}"; do
    for SERVICE in "${SERVICES[@]}"; do
        ...
        echo "{\"service\": \"$SERVICE\", \"role\": \"$ROLE\", \"status\": \"$STATUS\", \"elapsed_seconds\": $ELAPSED, \"evomaster_version\": \"$EVOMASTER_VERSION\"}" >> "$SUMMARY_FILE"
    done
done
